...
Ctrl + U: Copy selected username
Ctrl + P: Copy selected password
Ctrl + Q: Quit from program
Enter: Submit
...
impl PassMng {
    ...
    pub fn search(&mut self) {
        self.search_list = self.passwords.clone().into_iter()
            .filter(|item| item.title.starts_with(&self.search_txt.to_owned()) || item.title.contains(&self.search_txt.to_owned()) || item.username.starts_with(&self.search_txt.to_owned()) || item.username.contains(&self.search_txt.to_owned()) || item.password.starts_with(&self.search_txt.to_owned()) || item.password.contains(&self.search_txt.to_owned()))
            .collect();
    }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("\n\nWarning: don't forgot your passphrase!, you can't recover your passwords without it!\n");
    let passphrase = rpassword::prompt_password("Enter Passphrase (Enter for nothing): ").unwrap();
